 ...
 *
 * Ключевая идея: каждый поток выполняет ПОЛНОЕ количество
 * итераций (count_max), что увеличивает общее количество
 * исследованных комбинаций пропорционально числу потоков.
 * Это обеспечивает лучшее качество поиска при большей скорости.
 *
 * @return минимальная достигнутая ошибка, или big если не найдено
 */
float rndrod4_parallel() {
    ...
    const int MIN_ITERATIONS_PER_THREAD = 100;

    // Используем однопоточную версию если общее количество итераций
    // слишком мало для эффективного распределения между потоками.
    // Порог = MIN_ITERATIONS_PER_THREAD * NumThreads
    // Это гарантирует, что каждый поток получит минимум MIN_ITERATIONS_PER_THREAD итераций.
    int min_total_iterations = MIN_ITERATIONS_PER_THREAD * NumThreads;
    if (count_max < min_total_iterations) {
        return rndrod4();
    }

    // Каждый поток выполняет ПОЛНОЕ количество итераций (count_max),
    // что увеличивает общее количество исследованных комбинаций
    // в NumThreads раз. Это обеспечивает:
    // 1. Лучшее качество поиска (больше комбинаций исследовано)
    // 2. Эффективное использование всех доступных ядер
    // 3. Масштабируемость: больше потоков = больше исследованных комбинаций
    int iterations_per_thread = count_max;
